...
public abstract class SSLContextImpl extends SSLContextSpi {
    ...
    protected void engineInit(KeyManager[] km, TrustManager[] tm,
        ...
        if (sr == null) {
            ...
            if (Legacy8uJSSE.isFIPS() &&
                        (sr.getProvider() != Legacy8uJSSE.cryptoProvider)) {
                throw new KeyManagementException
                    ("FIPS mode: SecureRandom must be from provider "
                    + Legacy8uJSSE.cryptoProvider.getName());
            }
            secureRandom = sr;
        }
        ...
        for (int i = 0; tm != null && i < tm.length; i++) {
            if (tm[i] instanceof X509TrustManager) {
                if (Legacy8uJSSE.isFIPS() &&
                    ...
                    throw new KeyManagementException
                        ("FIPS mode: only Legacy8uJSSE TrustManagers may be used");
                }
    ...
    private X509ExtendedKeyManager chooseKeyManager(KeyManager[] kms)
        ...
        for (int i = 0; kms != null && i < kms.length; i++) {
            ...
            if (Legacy8uJSSE.isFIPS()) {
                // In FIPS mode, require that one of Legacy8uJSSE's own keymanagers
                ...
                if ((km instanceof X509KeyManagerImpl)
                    ...
                    throw new KeyManagementException
                        ("FIPS mode: only Legacy8uJSSE KeyManagers may be used");
                }
            }
     ...
     *
     * Considering above interoperability issues, Legacy8uJSSE will not set
     ...
     * SSL/TLS clients. In Legacy8uJSSE, TLS 1.1 or later version will be the
