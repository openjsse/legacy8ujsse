...
final class EllipticCurvesExtension extends HelloExtension {
    ...
    static {
        boolean requireFips = Legacy8uJSSE.isFIPS();
    ...
    private static boolean isPermitted(int curveId,
                AlgorithmConstraints constraints) {
        return constraints.permits(
                    EnumSet.of(CryptoPrimitive.KEY_AGREEMENT),
                                getCurveName(curveId), null) &&
                constraints.permits(
                    EnumSet.of(CryptoPrimitive.KEY_AGREEMENT),
                                "EC", idToParams.get(curveId));
    }

    static boolean isPermitted(String name,
                AlgorithmConstraints constraints) {
        NamedEllipticCurve nec =
                NamedEllipticCurve.getCurve(name, false);
    ...
    private static int getPreferredCurve(int[] curves,
        ...
        for (int curveId : curves) {
            if (isSupported(curveId)
                && isPermitted(curveId, constraints)) {
                return curveId;
            }
        }
