 ...
 *
 * Legacy8uJSSE now supports an experimental FIPS compliant mode when used with an
 ...
 *
 * It is currently not possible to use both FIPS compliant Legacy8uJSSE and
 ...
 * Security.removeProvider("Legacy8uJSSE") to work, which creates an instance of
 ...
 * data structures need to be initialized or until Legacy8uJSSE is initialized in
 * FIPS mode.
 *
 */
public abstract class Legacy8uJSSE extends java.security.Provider {
        ...
        if (fips == null) {
            ...
            if (fips == false) {
                throw new ProviderException
                    ("Legacy8uJSSE already initialized in non-FIPS mode");
            }
            if (cryptoProvider != p) {
                throw new ProviderException
                    ("Legacy8uJSSE already initialized with FIPS crypto provider "
                    + cryptoProvider);
            }
        }
    }
    ...
    private Legacy8uJSSE(java.security.Provider cryptoProvider,
                         String providerName) {
        super("Legacy8uJSSE", 1.8d, fipsInfo + providerName + ")");
        subclassCheck();
    ...
    private void doRegister(boolean isfips) {
        ...
        put("Signature.MD5andSHA1withRSA",
            "org.openjsse.legacy8ujsse.sun.security.ssl.RSASignature");

        put("KeyManagerFactory.SunX509",
            "org.openjsse.legacy8ujsse.sun.security.ssl.KeyManagerFactoryImpl$SunX509");
        put("KeyManagerFactory.NewSunX509",
            "org.openjsse.legacy8ujsse.sun.security.ssl.KeyManagerFactoryImpl$X509");
        put("Alg.Alias.KeyManagerFactory.PKIX", "NewSunX509");

        put("TrustManagerFactory.SunX509",
            "org.openjsse.legacy8ujsse.sun.security.ssl.TrustManagerFactoryImpl$SimpleFactory");
        put("TrustManagerFactory.PKIX",
            "org.openjsse.legacy8ujsse.sun.security.ssl.TrustManagerFactoryImpl$PKIXFactory");
        put("Alg.Alias.TrustManagerFactory.SunPKIX", "PKIX");
        put("Alg.Alias.TrustManagerFactory.X509", "PKIX");
        put("Alg.Alias.TrustManagerFactory.X.509", "PKIX");

        put("SSLContext.TLSv1",
            "org.openjsse.legacy8ujsse.sun.security.ssl.SSLContextImpl$TLS10Context");
        put("SSLContext.TLSv1.1",
            "org.openjsse.legacy8ujsse.sun.security.ssl.SSLContextImpl$TLS11Context");
        put("SSLContext.TLSv1.2",
            "org.openjsse.legacy8ujsse.sun.security.ssl.SSLContextImpl$TLS12Context");
        put("SSLContext.TLS",
            "org.openjsse.legacy8ujsse.sun.security.ssl.SSLContextImpl$TLSContext");
        if (isfips == false) {
        ...
        put("SSLContext.Default",
            "org.openjsse.legacy8ujsse.sun.security.ssl.SSLContextImpl$DefaultSSLContext");
    ...
    private void subclassCheck() {
        if (getClass() != org.openjsse.legacy8ujsse.net.ssl.Legacy8uJSSE.class) {
