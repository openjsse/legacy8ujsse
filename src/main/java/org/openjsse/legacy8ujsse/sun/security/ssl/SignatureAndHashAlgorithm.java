...
final class SignatureAndHashAlgorithm {
    ...
    private SignatureAndHashAlgorithm(String name, HashAlgorithm hash,
            SignatureAlgorithm signature, String algorithm,
            String curve, int priority) {
        this.name = name;
        this.hash = hash;
    ...
    private boolean isPermitted(AlgorithmConstraints constraints) {
        return constraints.permits(SIGNATURE_PRIMITIVE_SET,
               ...
               constraints.permits(SIGNATURE_PRIMITIVE_SET,
                        this.algorithm, null) &&
               (curve != null ?
                        EllipticCurvesExtension.isPermitted(curve,
                              constraints) : true);
    }
    ...
    static Collection<SignatureAndHashAlgorithm>
        ...
        for (SignatureAndHashAlgorithm sigAlg : priorityMap.values()) {
            if (sigAlg.priority <= SUPPORTED_ALG_PRIORITY_MAX_NUM &&
                    sigAlg.isPermitted(constraints)) {
    ...
    private static void supports(String name, HashAlgorithm hash,
            SignatureAlgorithm signature, String algorithm, int priority) {
            supports(name, hash, signature, algorithm, null, priority);
    }

    private static void supports(String name, HashAlgorithm hash,
            SignatureAlgorithm signature, String algorithm, String curve, int priority) {

        SignatureAndHashAlgorithm pair =
            new SignatureAndHashAlgorithm(name, hash, signature, algorithm, curve, priority);
    ...
    static SignatureAndHashAlgorithm getPreferableAlgorithm(
        ...
        return SignatureAndHashAlgorithm.getPreferableAlgorithm(
                algorithms, constraints, expected, null);
    }
    ...
    static {
        ...
        synchronized (supportedMap) {
            ...
            supports("dsa_sha256",
                    ...
                    "SHA256withDSA",        --p);
            supports("rsa_pkcs1_sha256",
                    HashAlgorithm.SHA256,      SignatureAlgorithm.RSA,
                    "SHA256withRSA",        --p);
            supports("ecdsa_secp256r1_sha256",
                    HashAlgorithm.SHA256,      SignatureAlgorithm.ECDSA,
                    "SHA256withECDSA", "secp256r1",      --p);
            supports("rsa_pkcs1_sha384",
                    HashAlgorithm.SHA384,      SignatureAlgorithm.RSA,
                    "SHA384withRSA",        --p);
            supports("ecdsa_secp384r1_sha384",
                    HashAlgorithm.SHA384,      SignatureAlgorithm.ECDSA,
                    "SHA384withECDSA", "secp384r1",      --p);
            supports("rsa_pkcs1_sha512",
                    HashAlgorithm.SHA512,      SignatureAlgorithm.RSA,
                    "SHA512withRSA",        --p);
            supports("ecdsa_secp521r1_sha512",
                    HashAlgorithm.SHA512,      SignatureAlgorithm.ECDSA,
                    "SHA512withECDSA", "secp521r1",      --p);
        }
    }
}
